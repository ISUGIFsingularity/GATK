...
if [ "$#" -eq 3 ]; then
  ...
  ${GENMODgit}/wrappers/GATK bwa mem -M -t ${THREADS} ${REF} ${READ1} ${READ2} | samtools view -buS - > ${OUTNAME}.bam
# if not just use the reads as single reads
elif [ "$#" -eq 1 ]; then
  READ1="$2"
  OUTNAME=$(basename ${READ1%.*} | cut -f 1-2 -d "_")
  ${GENMODgit}/wrappers/GATK bwa mem -M -t ${THREADS} ${REF} ${READ1} | samtools view -buS - > ${OUTNAME}.bam
